...
class GradientDescentTrainerForVAT(Trainer):
    """
        ...
        for batch_group in batch_group_generator_tqdm:
            ...
            for batch in batch_group:
                ...
                if epoch > self.delay_epoch:
                    vat_loss = VATLoss(cuda_device=self.cuda_device, xi=self.xi, eps=self.eps, ip=1,
                                       model=self._pytorch_model)

                    # LDS should be calculated before the forward for cross entropy
                    ul_batch = copy.deepcopy(batch)
                    del ul_batch['input_tensor']
                    ul_batch["input_tensor"] = ul_batch.pop("ul_input_tensor")
                    ul_batch = nn_util.move_to_device(ul_batch, self.cuda_device)
                    lds = vat_loss(ul_batch)
                del batch['ul_input_tensor']
